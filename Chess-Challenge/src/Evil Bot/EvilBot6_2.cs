...
{
    public class EvilBot6_2 : IChessBot
    {
        ...
        ushort[] killers = new ushort[2048];
        ...
        public int AlphaBeta(int alpha, int beta, int depth, int ply)
        {
            ...
            for (int i = 0; i < moves.Length; i++)
            {
                ...
                : 0);// + ((board.SquareIsAttackedByOpponent(move.TargetSquare) ? -100 : +100));
            }
            ...
            for (int i = 0; i < moves.Length; i++)
            {
                ...
                Move move = moves[i];
                board.MakeMove(move);
                int extension = qsearch ? 0 : (board.IsInCheck() ? 1 : ((move.MovePieceType == PieceType.Pawn && (move.TargetSquare.Rank == 6 || move.TargetSquare.Rank == 1)) ? 1 : 0));
                int score = -AlphaBeta(-(alpha + 1), -alpha, depth - 1 + extension, ply + 1);
                if (score > alpha && score < beta)
                {
                    score = Math.Max(score, -AlphaBeta(-beta, -alpha, depth - 1 + extension, ply + 1));
                }
                board.UndoMove(move);
        ...
        public Move Think(Board board, Timer timer)
        {
            ...
            Console.WriteLine("EvilBot: " + eval / -100f + ";\tDepth: " + calculatedDepth); //#DEBUG
