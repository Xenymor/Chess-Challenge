...
public class MyBot : IChessBot
{
    ...
    public Board board;
    const int DEPTH = 40;
    private const int TIME_DIVIDER = 15;
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        int timeForMove = timer.MillisecondsRemaining / TIME_DIVIDER;
        this.board = board;
        int alpha = -25;
        int beta = 25;
        bool broke = false;
        ...
        {
            lastEval = alphaBeta(alpha, beta, currentDepth, true, timer, timeForMove);
            if (timer.MillisecondsElapsedThisTurn >= timeForMove || lastEval < -CHECKMATE_SCORE+10 || lastEval > CHECKMATE_SCORE-10) {
                broke = true; //#DEBUG
                break;
            }
            if (lastEval <= alpha)
                alpha -= 50;
            else if (lastEval >= beta)
                beta += 50;
            else
            {
                currentDepth++;
        ...
        if (!broke) //#DEBUG
            currentDepth = DEPTH; //#DEBUG
        MatchStatsUI.movesPlayed1++; //#DEBUG
        MatchStatsUI.depthSum1 += currentDepth; //#DEBUG
        Console.WriteLine("MyBot: " + lastEval / 100d + ";\tdepth: " + (currentDepth+1) + ";\tMove: " + gameLength); //#DEBUG
        return bestRootMove;
    }

    int[][] scorePool;

    private int alphaBeta(int alpha, int beta, int depth, bool isFirstCall, Timer timer, long timeForMove)
    {
        if (!isFirstCall)
        ...
        for (byte i = 0; i < moves.Length; i++)
        {
            ...
            Move move = moves[i];
            board.MakeMove(move);
            int score = -alphaBeta(-beta, -alpha, depth - 1, false, timer, timeForMove);
            board.UndoMove(move);
            if (score >= beta)
            {
        ...
        if (isFirstCall && timer.MillisecondsElapsedThisTurn < timeForMove)
        {
            bestRootMove = bestMove;
        }
        return bestScore;
